...
public:
	...
	ULONG GetNumBytesRead() { return m_BytesRead; }
...
private:
    ...
    static const UINT NumBuffers = 20;					// Number of audio buffers (chunks of overall ring buffer) used to capture Kinect audio data
    typedef std::stack<StaticMediaBuffer*> BufferPool;
    typedef std::queue<StaticMediaBuffer*> CircularBufferQueue;
    

    UINT                    m_cRef;						// Number of references to this object
    IMediaObject*           m_pKinectDmo;				// Media object used to capture audio
    HANDLE                  m_hStopEvent;				// Event used to signal that capture thread should stop capturing audio
    HANDLE                  m_hDataReady;				// Event used to signal that there's captured audio data ready to be read
    HANDLE                  m_hCaptureThread;			// Audio capture thread
    BufferPool              m_BufferPool;				// Pool of unused buffers ready to be used for writing captured audio data
    CircularBufferQueue     m_ReadBufferQueue;			// Circular buffer queue that contains audio data ready for reading by stream clients
    StaticMediaBuffer*		m_CurrentWriteBuffer;		// Buffer where most recently captured audio data is being written
    StaticMediaBuffer*		m_CurrentReadBuffer;		// Buffer from which stream client is currently reading audio data
    ULONG                   m_CurrentReadBufferIndex;	// Next index to be read within current read buffer
    ULONG                   m_BytesRead;				// Total number of bytes read so far by audio stream client
    CRITICAL_SECTION        m_Lock;						// Critical section used to synchronize multithreaded access to captured audio data
    ...
    void                    QueueCapturedBuffer(StaticMediaBuffer *pBuffer);
