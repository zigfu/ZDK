...
public class ZigInputKinectSDK : IZigInputReader
{
    ...
    public void Init(ZigInputSettings settings)
    {
        ...
        {
            ZigFaceTracker.Instance.Initialize(Image, Depth, IntPtr.Zero, IntPtr.Zero); 
        }
    }
    ...
    public void Update()
    {
        ...
        if (UpdateImage)
        {
            ...
            {
                if (FaceTrackingEnabled) { ZigFaceTracker.Instance.UpdateVideoFrame(pImageFrame); }
        ...
        if (UpdateDepth)
        {
            ...
            {
                if (FaceTrackingEnabled) { ZigFaceTracker.Instance.UpdateDepthFrame(pDepthFrame); }
        ...
        if (FaceTrackingEnabled) { ZigFaceTracker.Instance.Track(); }
    }

    public void Shutdown()
    {
        ShutDownAudioAndSpeech();   // It's necessary to do this before NuiShutdown to prevent hang in Unity.

        NuiWrapper.NuiShutdown();
    }

    void ShutDownAudioAndSpeech()
    {
        if (Zigfu.Speech.ZigKinectSpeechRecognizer.InstanceExists)
            { Zigfu.Speech.ZigKinectSpeechRecognizer.Instance.Shutdown(); }

        if (Zigfu.KinectAudio.ZigKinectAudioSource.InstanceExists)
            { Zigfu.KinectAudio.ZigKinectAudioSource.Instance.Shutdown(); }
    }
